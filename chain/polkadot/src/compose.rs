...
/// Generates the polkadot's call field for a given module and call passed as &str
/// # Arguments
///
...
/// Generates an Unchecked polkadot for a given call
/// # Arguments
///
/// * 'signer' - AccountKey that is used to sign the polkadot.
/// * 'call' - call as returned by the compose_call! macro or via substrate's call enums.
/// * 'nonce' - signer's account nonce: u32
/// * 'era' - Era for polkadot to be valid
...
/// Generates an Unchecked polkadot for a given module and call passed as a &str.
/// # Arguments
///
/// * 'api' - This instance of API. If the *signer* field is not set, an unsigned polkadot will be generated.
/// * 'module' - Module name as &str for which the call is composed.
/// * 'call' - Call name as &str
/// * 'args' - Optional sequence of arguments of the call. They are not checked against the metadata.
/// As of now the user needs to check himself that the correct arguments are supplied.
...
macro_rules! compose_extrinsic {
	...
	$(, $args: expr) *) => {
		{
            #[allow(unused_imports)] // For when polkadot does not use Compact
            use $crate::codec::Compact;
            use $crate::log::debug;
            use $crate::primitives::UncheckedExtrinsicV4;
            use $crate::sp_runtime::generic::Era;

            debug!("Composing generic polkadot for module {:?} and call {:?}", $module, $call);
